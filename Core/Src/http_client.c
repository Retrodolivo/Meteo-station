...
static bool link_was_off = false;
...
uint8_t httpc_connection_handler()
{
	...
	if (wizphy_getphylink() == PHY_LINK_OFF)
	{
		link_was_off = true;
		httpc_isConnected = HTTPC_FALSE;
		state = getSn_SR(httpsock);
	}
	else if (link_was_off && wizphy_getphylink() == PHY_LINK_ON)
	{
		link_was_off = false;
//		re_init_net();
	}
...
uint16_t httpc_add_customHeader_field(uint8_t * customHeader_buf, const char * name, const char * value)
{
	...
	if((strlen((char *)name) + strlen((char *)value)) > TX_BUF_SIZE) return 0;
...
uint16_t httpc_send_header(HttpRequest * req, uint8_t * buf, uint8_t * customHeader_buf, uint16_t content_len)
{
	...
	if(httpc_isConnected == HTTPC_TRUE)
	{
		...
		{
			if((strlen((char *)customHeader_buf) + len + 2) <= TX_BUF_SIZE)
			{
...
uint16_t httpc_send(HttpRequest * req, uint8_t * buf, uint8_t * body, uint16_t content_len)
{
	...
	if(httpc_isConnected == HTTPC_TRUE)
	{
		do
		{
			...
			if((len + content_len) > TX_BUF_SIZE) {
				content_len = TX_BUF_SIZE - len; // HTTP request header re-generation (do-while)
...
// return: data lenght without content lenght
uint16_t httpc_form_req_no_body(HttpRequest * req, uint8_t * buf, uint16_t content_len)
{
	uint16_t len;
	uint8_t http_header_generated = HTTPC_FAILED;

	if (1)//if(httpc_isConnected == HTTPC_TRUE)
	{
		do
		{
			memset(buf, 0x00, TX_BUF_SIZE);
...
void http_send_buf(uint8_t *buf, uint16_t len)
{
	send(httpsock, buf, len);
}
